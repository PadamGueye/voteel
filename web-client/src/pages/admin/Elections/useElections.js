...
const useElections = () => {
    ...
    useEffect(() => {
        setLoading(true);
        ElectionAPI.getElections().then((res)=>{
            setUserList([]);
            res.data.map((election)=>{
                const [formattedDate, formattedTime] = formatDate(election?.createdAt);
                const mappedUser = {
                    name : election?.name,
                    status : election?.status,
                    date_creation : formattedDate,
                    heure_creation : formattedTime,
                    action : <UserActionColumn
                                onView={()=>{}}
                                //onEdit={()=>{navigate(`/edit-user/${user?.id}`)}}
                                onEdit={()=>{}}
                                onDelete={()=>{setShowMainModal(true); setSelectedUser(election?.id); setAction("delete")}}
