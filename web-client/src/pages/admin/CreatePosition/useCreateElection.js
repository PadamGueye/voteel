...
const useCreateElection = ()=>{
    const navigate = useNavigate();
    const [stateModal, setStateModal] = useState({show: false, message : "", title : "", type:""});
    const [showMainModal, setShowMainModal] = useState(false);
    const [preview, setPreview] = useState(false);
    const [action, setAction] = useState("");
    const emptyElectionData = {name : "",	status : ""};
    const [userData, setUserData] = useState(emptyElectionData);
    ...
    const onSubmit = async () => {
        ...
        if (action === "annuler") {
            ...
            ElectionAPI.addElection(formatedData).then(
                (res)=>{
                    setFormatedData({});
                    setUserData(emptyElectionData);
