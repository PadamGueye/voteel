    ...
    return (
        <MainLayout userRole={Types.EDITOR}>
            <div className={"px-10 py-4 flex flex-col items-center"}>
                ...
                <MainModal showModal={showMainModal} title={`Vous êtes sur le point ${action === "annuler" ? "d'annuler" : "de valider"} une réservation, êtes vous sûre de vouloir continuer ?`} onCancel={()=>{setShowMainModal(false); resetStateModal()}} onSubmit={()=>{(action === "annuler" ? handleRemove() : handleValidate()); setShowMainModal(false)}} />
